"""Bang bang control example for BionicMotor."""
...
# Initial conditions
def trapezoidal_profile(t: float, i_max: int, t_ramp: int) -> int:
    if t < t_ramp:
        # Ramp up
...
def calculate_motor_current(
    pos_desired: int, pos_current: int, spd_desired: int, spd_current: int, torque_ff: int, start_time: float
) -> int:
    # Time elapsed since start
    t = time.time() - start_time

    # PID-like control effort
    control_effort = (KP * (pos_desired - pos_current) + KD * (spd_desired - spd_current) + torque_ff) / KT
